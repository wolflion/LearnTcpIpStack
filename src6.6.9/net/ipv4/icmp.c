...
int icmp_rcv(struct sk_buff *skb)
{
	...
	icmph = icmp_hdr(skb); //丢弃ICMP首部，获得ICMP报文内容，lionel
	 ...
	 *		  discarded.
	 */
	if (icmph->type > NR_ICMP_TYPES) {  //检测ICMP报文的类型，其值超过现有类型的最大值，报错
		reason = SKB_DROP_REASON_UNHANDLED_PROTO;
		goto error;
	}

	/*
	 *	Parse the ICMP message  //通过广播或组播的方式发送ICMP报文
	 */
	...
	if (rt->rt_flags & (RTCF_BROADCAST | RTCF_MULTICAST)) {
		...
		if ((icmph->type == ICMP_ECHO ||
		    ...
		    READ_ONCE(net->ipv4.sysctl_icmp_echo_ignore_broadcasts)) {//开启icmp_echo_ignore_broadcasts时，组播或广播的是请求回显或时间戳时，被忽略
		...
		if (icmph->type != ICMP_ECHO &&
		    ...
		    icmph->type != ICMP_ADDRESSREPLY) {  //除了以上4种，其它的都不支持 组播或广播，前2种在上面判断过了，要不开启ingore_broadcasts时
			reason = SKB_DROP_REASON_INVALID_PROTO;
			goto error;
		}
	}

	reason = icmp_pointers[icmph->type].handler(skb);//找到类型的结构实例，调用处理函数处理icmp报文
reason_check:
	if (!reason)  {
		consume_skb(skb);
...
int __init icmp_init(void)
{
	...
	for_each_possible_cpu(i) {
		...
		err = inet_ctl_sock_create(&sk, PF_INET,
					   SOCK_RAW, IPPROTO_ICMP, &init_net);//lionel，这个地方封装了一下，比2.*版本改过，去掉了per_cpu()
		if (err < 0)
			return err;
