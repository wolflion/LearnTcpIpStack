...
{
	int ret;
	struct net *net = dev_net(dev);  //netdevice.h中的net-namespace有关
	...
	might_sleep(); //支持内核抢占后，需要检查 是否需要重新调度
 ...
 *	alloc_netdev.
 */
int register_netdev(struct net_device *dev)  //就是一个壳
{
	int err;
 ...
 */
static int __init net_dev_init(void) // __init不太熟悉
{
	int i, rc = -ENOMEM;

	BUG_ON(!dev_boot_phase);
    //dev.h中的，CONFIG_PROC_FS这个开关
	if (dev_proc_init())  //在proc文件系统中注册 proc/net/dev和proc/net/softnet_stat文件，
		goto out;
    // net/core/net-sysfs.c中的   【todo，这个需要什么基础知识？】
	if (netdev_kobject_init()) //todo，从原来的sysfs换成kobject模型了？
		goto out;

	INIT_LIST_HEAD(&ptype_all);  //网络处理函数?  [处理不同协议的报文？]  实现只是一个链表
	for (i = 0; i < PTYPE_HASH_SIZE; i++)
		INIT_LIST_HEAD(&ptype_base[i]);  // dev_add_pack()会加进去，把加进去的值，放到list上去  【顺序怎么办，最开始就是空吧】

	if (register_pernet_subsys(&netdev_net_ops)) //函数是net_namespace.c中的，
		goto out;
	...
	for_each_possible_cpu(i) {  //只是笼统的写了一下 初始化与CPU相关的接收队列， 这个其实跟softdata结构体有关
		struct work_struct *flush = per_cpu_ptr(&flush_works, i);
		struct softnet_data *sd = &per_cpu(softnet_data, i);  //todo，这个softnet_data的从哪来？全局的变量吗？ [dev.h中也没有]
	...
	dev_boot_phase = 0;  //todo，没说干啥的  【标识设备层初始化已完成】

	/* The loopback device is special if any other network devices   loopback的设备必须
	 ...
	 * that disappears.
	 */
	if (register_pernet_device(&loopback_net_ops))  //loopback.c中定义的，todo，两者怎么关联上的？
		goto out;

	if (register_pernet_device(&default_device_ops))
		goto out;

	open_softirq(NET_TX_SOFTIRQ, net_tx_action);  //虽然是open，但确是注册2个软中断
	open_softirq(NET_RX_SOFTIRQ, net_rx_action);
    //cpuhotplug.h中的，最后调用了cpu.c中的__cpuhp_setup_state_cpuslocked
	rc = cpuhp_setup_state_nocalls(CPUHP_NET_DEV_DEAD, "net/dev:dead",
				       NULL, dev_cpu_dead); //todo，不是很熟  【是不是把dev_cpu_dead注册进去啊？】
	WARN_ON(rc < 0);
	rc = 0;
out:
	return rc;
